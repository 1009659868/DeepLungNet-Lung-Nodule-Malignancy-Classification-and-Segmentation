<template>
	<view class="htz-image-upload-list">
		<view v-if="uploadLists.length>0" class="list-title-container">
			<view class="preview-title">
				图片预览
			</view>
		</view>
		<view class="upload-list-container">
			<view class="htz-image-upload-Item htz-image-upload-Item-show" v-for="(item,index) in uploadLists" :key="index">
				<view class="htz-image-upload-Item-video" v-if="isVideo(item)">
					...
					<image v-if="dataType==1 && item.cover" class="htz-image-upload-Item-video-app-poster"
						mode="widthFix" :src="item.cover"></image>
				...
				<view class="htz-image-upload-Item-index">{{index+1}}</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			imgUpload(tempFilePaths, type) { //type 0 为图片 1为视频
				...
				tempFilePaths.forEach((item, index) => {
					uploadImgs.push(new Promise((resolve, reject) => {
						...
						const uploadTask = uni.uploadFile({
							...
							fileType: 'image',
							formData: {
								// ...this.formData, // 其他表单字段
								fileType: Object,
								username: JSON.stringify(u_name) // 将用户信息转换为字符串
							},
							header: this.headers,
							success: (uploadFileRes) => {
								...
								if (typeof this.uploadSuccess ==
									'function') {

									let thisUploadSuccess = this
										.uploadSuccess(
											uploadFileRes)
									if (thisUploadSuccess.success) {
										...
										if (this.dataType == 0) {
											this[keyName].push(
												thisUploadSuccess.url)
										} else {
											this[keyName].push({
												type: type,
												url: thisUploadSuccess
													.url,
												...
												thisUploadSuccess
											})
							...
							fail: (err) => {
								...
								uni.showToast({
									...
									duration: 2000 // 提示持续时间为 2000 毫秒（2 秒）
								});
								uni.hideLoading();
								reject(err);
				...
				// 	key: 'userInfo',
				// 	success: (res) => {
				// 		const username = res.data.u_name
